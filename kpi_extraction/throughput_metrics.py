...
def get_throuput(cycle_times):
    ...
    return {
        "throughput-daily": float(round(throughput_per_day, 1)),
        "wip-mean": float(round(avg_wip, 1)),
        "wip-max": float(round(max_wip, 1)),
        "littles-law-est": float(round(wip_estimate_little, 1))
    }